/**
 * tivis_final.js — Multi-Portal Termux server (FINAL) with runtime IP patch
 * Merges: Stalker portals + external JStar M3U + 3 custom DRM channels
 *
 * Note: JStar URL preserved as raw M3U content and appended after portal channels.
 */
...
// External JStar M3U URL (kept as raw)
const JSTAR_URL = 'https://raw.githubusercontent.com/alex8875/m3u/main/jstar.m3u';
...
/* ---------------------- Helper: fetch external M3U and sanitize ---------------------- */

async function fetchExternalM3UText(url) {
  try {
    const r = await fetchWithAgent(url, { timeout: 10000 });
    if (!r || (r.status && (r.status < 200 || r.status >= 400))) {
      console.error('fetchExternalM3UText: bad status', r && r.status);
      return '';
    }
    ...
    txt = txt.replace(/\r\n/g, '\n').trim();
    // If it starts with #EXTM3U, remove that header line to avoid double header
    if (txt.startsWith('#EXTM3U')) {
      const parts = txt.split('\n');
      parts.shift();
      txt = parts.join('\n').trim();
    }
    // Return as-is (we keep group-title lines from source so groups preserved)
    return '\n# --- BEGIN EXTERNAL M3U: ' + url + ' ---\n' + txt + '\n# --- END EXTERNAL M3U ---\n';
  } catch (e) {
    console.error('fetchExternalM3UText error', e && e.message);
    return '';
  }
}
...
// --- Standalone JTV fetch (clean, no agent, no headers) ---
async function fetchJTVStandalone() {
  try {
    const r = await fetch("https://raw.githubusercontent.com/alex8875/m3u/main/jstar.m3u");
    if (!r.ok) return "";
    let txt = await r.text();
    txt = txt.replace(/^#EXTM3U.*/i, "").trim();
    return "\n# --- BEGIN JSTAR (Standalone) ---\n" + txt + "\n# --- END JSTAR ---\n";
  } catch (e) {
    console.log("JTV standalone fetch error:", e.message);
    return "";
  }
}
...
app.get('/playlist.m3u8', async (req,res) => {
  try {
    ...
    if (!token) {
      ...
      console.error('playlist preview: token generation failed, returning combined playlist with any available external sources.');
    }
    ...
    // --- Append JTV Standalone (matches working temp.js) ---
    const jtvBlock = await fetchJTVStandalone();
    if (jtvBlock) {
      m3u += jtvBlock;
    }
    // --- Append 3 custom DRM channels (always appended, not filtered) ---
    m3u += '\n# -------- CUSTOM CHANNELS (Not Filtered) --------\n';
    ...
    m3u += `#KODIPROP:inputstream.adaptive.license_key=40f019b86241d23ef075633fd7f1e927:058dec845bd340178a388edd104a015e\n`;
    m3u += `#EXTINF:-1 tvg-id="ts210" tvg-logo="https://ltsk-cdn.s3.eu-west-1.amazonaws.com/jumpstart/Temp_Live/cdn/HLS/Channel/transparentImages/MNPlusHD.png" group-title="Movies",MN+\n`;
    m3u += `https://times-ott-live.akamaized.net/mnplus_wv_drm/index.mpd\n\n`;

    m3u += `#KODIPROP:inputstreamaddon=inputstream.adaptive\n`;
    m3u += `#KODIPROP:inputstream.adaptive.manifest_type=dash\n`;
    m3u += `#KODIPROP:inputstream.adaptive.license_type=org.w3.clearkey\n`;
    m3u += `#KODIPROP:inputstream.adaptive.license_key=40f019b86241d23ef075633fd7f1e927:058dec845bd340178a388edd104a015e\n`;
    m3u += `#EXTINF:-1 tvg-id="ts599" tvg-logo="https://ltsk-cdn.s3.eu-west-1.amazonaws.com/jumpstart/Temp_Live/cdn/HLS/Channel/transparentImages/MNX%20HD.png" group-title="Movies",MNX\n`;
    m3u += `https://times-ott-live.akamaized.net/mnxhd_wv_drm/index.mpd\n\n`;

    m3u += `#KODIPROP:inputstreamaddon=inputstream.adaptive\n`;
    m3u += `#KODIPROP:inputstream.adaptive.manifest_type=dash\n`;
    m3u += `#KODIPROP:inputstream.adaptive.license_type=org.w3.clearkey\n`;
    m3u += `#KODIPROP:inputstream.adaptive.license_key=40f019b86241d23ef075633fd7f1e927:058dec845bd340178a388edd104a015e\n`;
    m3u += `#EXTINF:-1 tvg-id="ts173" tvg-logo="https://ltsk-cdn.s3.eu-west-1.amazonaws.com/jumpstart/Temp_Live/cdn/HLS/Channel/transparentImages/Movies%20Now%20HD.png" group-title="Movies",Movies Now\n`;
    m3u += `https://times-ott-live.akamaized.net/moviesnow_wv_drm/index.mpd\n\n`;

    m3u += `#EXTINF:-1 group-title="Extra",J-Star Playlist\n`;
    m3u += `https://raw.githubusercontent.com/alex8875/m3u/main/jstar.m3u\n`;
    m3u += '\n# -------- END CUSTOM --------\n';
...
`;
    ...
    // Insert infoBlock after #EXTM3U (first occurrence)
...
app.post('/portal/:portal/save', async (req,res) => {
  ...
  try {
    ...
    const { token, profile, account_info } = await portal_genTokenAndProfile(cfg);
    if (!token) {
      console.error('save playlist: token generation failed, proceeding with external merges where possible.');
    }
    const channels = await portal_getAllChannels(cfg, token);
    const genres = await portal_getGenres(cfg, token);
    const gmap = {}; (genres||[]).forEach(g=>{ if(g && g.id) gmap[g.id]=g.title; });
    ...
    // append external JStar
    try {
      const external = await fetchExternalM3UText(JSTAR_URL);
      if (external) m3u += '\n' + external;
    } catch (e) {
      console.error('Error fetching external JStar M3U (save)', e && e.message);
    }

    // append custom channels (same block as preview)
    m3u += '\n# -------- CUSTOM CHANNELS (Not Filtered) --------\n';
    m3u += `#KODIPROP:inputstreamaddon=inputstream.adaptive\n`;
    m3u += `#KODIPROP:inputstream.adaptive.manifest_type=dash\n`;
    m3u += `#KODIPROP:inputstream.adaptive.license_type=org.w3.clearkey\n`;
...
app.get('/portal/:portal/:id.m3u8', async (req,res) => {
  ...
  try {
    ...
    // fallback to default (fixed indentation below)
    if (!stream) {
        // Try stable commands first
        const stableTries = [
          `auto ${id}`,
          `ffmpeg http://localhost/ch/${id}`
        ];
        for (const t of stableTries) {
          stream = await portal_createLink(cfg, id, token, t);
          if (stream) break;
        }
    }

    // Final fallback to ffrt
...
app.listen(PORT, async () => {
  ...
  const net = getNetworkInfo();
  console.log('\n=============================================');
  console.log('   ✔ Multi-Portal Server Running (tivis_final.js)');
  console.log('=============================================');
